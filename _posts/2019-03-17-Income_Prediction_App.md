...
``Education_Label`` and ``Education_Number`` The education level attained by individual in words and corresponding numeric value in `Education_Number` column where Pre school is considered as basic level of education with numeric value 1 and the largest numeric value 16 is for the highest level of education attained which is Doctorate,
